...
class OC_Util {
	...
	public static function checkServer() {
		...
		if(!is_writable(OC::$configDir) or !is_readable(OC::$configDir)) {
			$errors[] = array(
				...
				'hint' => $l->t('This can usually be fixed by '
					  .'<a href="%s" target="_blank">giving the webserver write access to the config directory</a>.',
					  array(OC_Helper::linkToDocs('admin-dir_permissions')))
				);
		}
		...
		if(OC_Config::getValue('appstoreenabled', true)) {
			if( OC_App::getInstallPath() === null
				...
				$errors[] = array(
					...
					'hint' => $l->t('This can usually be fixed by '
						  .'<a href="%s" target="_blank">giving the webserver write access to the apps directory</a>'
		...
		if(version_compare(phpversion(), '5.3.3', '<')) {
			$errors[] = array(
				'error'=> $l->t('PHP 5.3.3 or higher is required.'),
